...
const BAZAAR_ACCEPT_SCHEMAS: Record<string, { info: unknown; schema: unknown }> = {
  ...
  "/api/v1/trading/engine/orderbook-depth":   { info: { input: { pair: "BTC/USDT" } }, schema: { type: "object", properties: { pair: { type: "string" } } } },
  ...
  "/api/v1/coding/cache/security-audit":      { info: { input: { code: "db.query(`SELECT * FROM users WHERE id=${req.id}`)" } }, schema: { type: "object", required: ["code"], properties: { code: { type: "string" }, language: { type: "string" } } } },
