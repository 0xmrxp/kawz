...
const ROUTE_INPUT_SCHEMAS: Record<string, unknown> = {
  ...
  "/api/v1/trading/engine/gas-tracker":         { type: "object", properties: { networks: { type: "string", description: "Comma-separated: eth,base,sol. Omit for all." } } },
  ...
  // Web Intelligence
  "/api/v1/web/intelligence/screenshot":        { type: "object", required: ["url"], properties: { url: { type: "string" }, width: { type: "number", default: 1280 }, height: { type: "number", default: 800 } } },
...
const BAZAAR_ACCEPT_SCHEMAS: Record<string, unknown> = {
  ...
  "/api/v1/trading/engine/gas-tracker":       declareDiscoveryExtension({ output: { example: { base: { slow: 0.001, standard: 0.002, fast: 0.005, unit: "gwei" }, eth: { slow: 8, standard: 15, fast: 35, unit: "gwei" }, solana: { low: 1000, medium: 5000, high: 25000, unit: "microlamports" } } } }).bazaar,
  ...
  "/api/v1/web/intelligence/screenshot":      declareDiscoveryExtension({ input: { url: "https://example.com", width: 1280, height: 800 }, inputSchema: { properties: { url: { type: "string" }, width: { type: "number" }, height: { type: "number" } }, required: ["url"] }, output: { example: { url: "https://example.com", screenshot_base64: "iVBORw0KGgo...", format: "png", viewport: { width: 1280, height: 800 } } } }).bazaar,
